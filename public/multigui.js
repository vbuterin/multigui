...
function MultiguiCtrl($scope,$http) {
    ...
    //Handles the key management side
    $scope.calculateBrainwallet = function() {
    ...
    $scope.$watch('keygen',function(newobj,oldobj) {
        ...
        if (newobj.priv) {
            newobj.pub = privtopub(newobj.priv);
            newobj.address = pubkey_to_address(newobj.pub);
        }
        else {
            newobj.pub = newobj.address = '';
        }
    },true);
    ...
    $scope.convertKey = function(obj,prop,cb) {
        ...
        // Is an address
            ...
            var addr = obj[prop];
            $scope.inprogress("Attempting to recover public key from the blockchain");
    ...
    $scope.getMultiAddr = function() {
        ...
        var k = $scope.msiginp.k,
            n = pubs.filter(function(x) { return x }).length;
            pubs.sort(function(x,y) { return x>y });

        if (!k || n == 0 || k > n) { 
            return $scope.msig = {};
        }
        var script = [k].concat(pubs).concat([n,'CHECKMULTISIG']),
